...
PROJECTS = {
    "OSP": {
        ...
        "fixed": {
            "data": 3, "pvol": 2, "rely": 5, "pcap": 3, "plex": 3, "site": 3, "time": 3 
        }
    },
    "OSP2": {
        "ranges": {
...
def assess(row, defs, risk_table=None, baseline=None, epsilon=0.5):
  ...
  # NEW: Unit Cost (Effort / LOC)
  unit_cost = effort / vals["LOC+"]
  ...
  # Return [Inputs..., Risk-, Change-, Effort(No Sign), UnitCost-]
  return clean + [risk, change, effort, unit_cost]
...
def cohens_d(mu1, sd1, mu2, sd2):
  denominator = sd1 if sd1 > 1e-9 else 1
...
def analyze_project(proj_name, m=100, r=20, epsilon=0.5):
    ...
    # Headers updated: Risk-, Change-, Effort (Passive), UnitCost- (Active)
    rows = [assess(inp, defs, risk_table, baseline=baseline_means, epsilon=epsilon) for inp in raw_inputs]
    header = [n for n,_,_,_ in defs] + ["Risk-", "Change-", "Effort", "UnitCost-"]
    ...
    metrics = [d[0] for d in defs] + ["Risk-", "Change-", "Effort", "UnitCost-"]
    
    for i, name in enumerate(metrics):
        base_val = initial_stats[name][0]
        base_sd  = initial_stats[name][1]
        ...
        d = cohens_d(base_val, base_sd, lrn_val, lrn_sd)
        
        if base_sd < 0.01:
             is_diff = abs(lrn_disp - base_disp) > 0.01
        else:
             is_diff = abs(d) >= 0.35
             
        if name == "Change-": is_diff = True

        lrn_str = f"{lrn_disp:8.2f}" if is_diff else f"{'=':>8}"
        ...
        for p_idx, res in enumerate(policy_results):
            ...
            if abs(val_disp - base_disp) < 0.05:
                pol_str = f"{'=':>6}"
            else:
