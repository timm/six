...
def Num():
    return obj(it=Num, n=0, mu=0, m2=0)
...
def adds(src, i=None):  # (src:Iterable, ?i) -> i
    i = i or Num()
    [add(i, v) for v in src]
    return i


def sub(i, v):
    return add(i, v, inc=False)


def add(i, v, inc=True):
    if v != "?":
        if Data is i.it and not i.cols:
            i.cols = Cols(v)  # initializing, not adding
        else:
            i.n += 1  # adding
            if Sym is i.it:
                ...
                [
                    add(col, v[col.at], inc) for col in i.cols.all
                ]  # recursive add to cols
...
## Cutting -------------------------------------------------------------------
def score(num):
    return num.mu + sd(num) / (sqrt(num.n) + 1 / BIG)
...
def cuts(col, rows, data):
    ...
    for x, y in sorted(xys):
        ...
        if k not in d:
            d[k] = obj(at=col.at, txt=col.txt, xlo=x, xhi=x, y=Num())
        add(d[k].y, y)
        d[k].xhi = x
...
### Main ---------------------------------------------------------------------
def select(rule, row):
    if (x := row[rule.at]) == "?" or rule.xlo == rule.xhi == x:
        return True
...
def xai(data):
    ...
    def go(rows, lvl=0, prefix=""):
        ys = Num()
        rows.sort(key=lambda row: add(ys, disty(data, row)))
        print(
            f"{o(rows[len(rows) // 2])}: {o(mu=ys.mu, n=ys.n, sd=sd(ys)):25s} {prefix}"
        )
        if rule := cut(data, rows):
            ...
            if 4 < len(now) < len(rows):
                go(
                    now,
                    lvl + 1,
                    f"{'|.. ' * lvl}{rule.txt} {o(rule.xlo)}..{o(rule.xhi)} ",
                )
...
def o(v=None, dec=2, **d):
    ...
    if isa(v, list):
        return f"[{', '.join(o(k, dec) for k in v)}]"
    if isa(v, tuple):
        return f"({', '.join(o(k, dec) for k in v)})"
    if callable(v):
        return v.__name__
    if hasattr(v, "__dict__"):
        v = vars(v)
    if isa(v, dict):
        return "{" + " ".join(f":{k} {o(v[k], dec)}" for k in v) + "}"
    return str(v)
